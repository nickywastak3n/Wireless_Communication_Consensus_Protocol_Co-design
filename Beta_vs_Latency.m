...
beta = 0.0001:0.1:5.0001; % Acceptable consensus distortion
gamma = 0.9;
...
N_beta_gossip_center = 1./(1/N+beta.^2*N/2/g_inverse(gamma)^2/psi_gossip_center);
N_tilda_gossip_center = ceil(N_beta_gossip_center);
N_beta_gossip_corner = 1./(1/N+beta.^2*N/2/g_inverse(gamma)^2/psi_gossip_corner);
N_tilda_gossip_corner = ceil(N_beta_gossip_corner);
...
N_beta_broadcast_center = 1./(1/N+beta.^2*N/2/g_inverse(gamma)^2/psi_broadcast_center);
N_tilda_broadcast_center = ceil(N_beta_broadcast_center);
N_beta_broadcast_corner = 1./(1/N+beta.^2*N/2/g_inverse(gamma)^2/psi_broadcast_corner);
N_tilda_broadcast_corner = ceil(N_beta_broadcast_corner);
...
% Figure settings
figure;
title('E2E Latency vs Acceptable Consensus Distortion');
xlabel('Acceptable Consensus Distortion (beta)');
...
plot(0.0, E2E_RC_g, 'bo', 'DisplayName', 'RC(G) Analy.');
plot(beta, E2E_R2C_g_cen,'b--', 'DisplayName', 'R2C(G) Analy.');
plot(beta, E2E_R2C_g_cor,'b--', 'HandleVisibility','off');
plot(0.0, E2E_RC_b, 'ro', 'DisplayName', 'RC(B) Analy.');
plot(beta, E2E_R2C_b_cen,'r--', 'DisplayName', 'R2C(B) Analy.');
plot(beta, E2E_R2C_b_cor,'r--', 'HandleVisibility','off');

plot(0.0, E2E_RC_g_exp,"co", 'DisplayName', 'RC(G) Exp.');
plot(beta, E2E_R2C_g_cen_exp,"c--", 'DisplayName', 'R2C(G) Exp.');
plot(beta, E2E_R2C_g_cor_exp,"c--", 'HandleVisibility','off');
plot(0.0, E2E_RC_b_exp,"mo", 'DisplayName', 'RC(B) Exp.');
plot(beta, E2E_R2C_b_cen_exp,"m--", 'DisplayName', 'R2C(B) Exp.');
plot(beta, E2E_R2C_b_cor_exp,"m--", 'HandleVisibility','off');
