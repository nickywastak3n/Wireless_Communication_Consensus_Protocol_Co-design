...
for source=1:N+1 
    dissemination_time_gossip_matrix(source) = dissemination_energy_gossip(N,source,outage_probability_g);
...
% Figure settings
figure;
title('Energy Consumption vs # of Faulty Nodes');
...
%plot(F, E2E_RC_g*ones(1,size(F,2))*transmitPower_g, 'bo', 'DisplayName', 'RC(G) Analy.');
%plot(F, E2E_R2C_g_cen*transmitPower_g,'b--', 'DisplayName', 'R2C(G) Analy. (Cen)');
%plot(F, E2E_R2C_g_cor*transmitPower_g*20,'b--', 'DisplayName', 'R2C(G) Analy. (Cor)');
%plot(F, E2E_RC_b*ones(1,size(F,2))*transmitPower_b, 'ro', 'DisplayName', 'RC(B) Analy.');
%plot(F, E2E_R2C_b_cen*transmitPower_b,'r--', 'DisplayName', 'R2C(B) Analy. (Cen)');
%plot(F, E2E_R2C_b_cor*transmitPower_b,'r--', 'DisplayName', 'R2C(B) Analy. (Cor)');

plot(F, E2E_RC_g_exp*ones(1,size(F,2))*transmitPower_g,"co", 'DisplayName', 'RC(G) Exp.');
plot(F, E2E_R2C_g_cen_exp*transmitPower_g,"c--", 'DisplayName', 'R2C(G) Exp.');
plot(F, E2E_R2C_g_cor_exp*transmitPower_g,"c--", 'HandleVisibility','off');
plot(F, E2E_RC_b_exp*ones(1,size(F,2))*transmitPower_b,"mo", 'DisplayName', 'RC(B) Exp.');
plot(F, E2E_R2C_b_cen_exp*transmitPower_b,"m--", 'DisplayName', 'R2C(B) Exp.');
plot(F, E2E_R2C_b_cor_exp*transmitPower_b,"m--", 'HandleVisibility','off');
