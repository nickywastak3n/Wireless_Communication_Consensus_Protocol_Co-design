...
E2E_R2C_g = zeros(1, length(N_vector));
E2E_R2C_b = zeros(1, length(N_vector));
E2E_R2C_g_exp = zeros(1, length(N_vector));
E2E_R2C_b_exp = zeros(1, length(N_vector));
...
for n=1:length(N_vector)
    ...
    %==============================
    % R2C gossip analysis
    E2E_R2C_g(n) = (3/2*sqrt(N+1)-1)*N_tilda+sqrt(N+1)-1; % E2E latency with proposer in center
    ...
    E2E_R2C_b(n) = N_tilda/N*E2E_RC_b + (N-N_tilda)/N*ceil((log(1-zeta^(1/N)))/log(outage_probability_max_matrix(proposer)));
    ...
    E2E_R2C_g_exp(n)= dissemination_time_gossip_matrix(proposer) + sum(dissemination_time_gossip_matrix(validator_set));
    % R2C broadcast experiment
    E2E_R2C_b_exp(n) = dissemination_time_broadcast_matrix(proposer) + sum(dissemination_time_broadcast_matrix(validator_set));
end

%==============================
% Figure settings
figure;
title('E2E Latency vs Number of nodes');
...
plot(N_vector, E2E_R2C_g, 'b--', 'DisplayName', 'R2C(G) Analy.');
plot(N_vector, E2E_R2C_b, 'r--', 'DisplayName', 'R2C(B) Analy.');
plot(N_vector, E2E_R2C_g_exp, 'c--', 'DisplayName', 'R2C(G) Exp.');
plot(N_vector, E2E_R2C_b_exp, 'm--', 'DisplayName', 'R2C(B) Exp.');
